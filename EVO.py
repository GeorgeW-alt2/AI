#Spiking neural network (SNNs) EVO 5.0 - George W - 5,12,2024
import numpy as np
import pickle
import re
import random
...
def main():
    ...
    while True:
        user_input = generate_text(model, word_to_index, ' '.join(reversed(user_input.split())), sequence_length=4, generate_length=generate_length, temperature=temperature)
        if user_input.find(".") < 100 and user_input.find(".") > 10:
            print("AI:", user_input[:user_input.find(".")])  # Prints the text before the first period
