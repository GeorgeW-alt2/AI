#Factoid 3.0 - George W - 29,NOV,2024
import numpy as np
import pickle
import re
import random
...
def save_model_and_vocab(model, word_to_index):
    torch.save(model.state_dict(), 'factoid.mdl')
    with open('factoid_vocab.pkl', 'wb') as f:
        pickle.dump(word_to_index, f)
    print("Model and vocabulary saved.")

def load_model_and_vocab(vocab_path='factoid_vocab.pkl', model_path='factoid.mdl'):
...
def main():
    ...
    if choice == '1':
        ...
        sequences_first_half = sequences[:split_idx]
        sequences_second_half = sequences[split_idx:]

        dataset_first_half = TextDataset(sequences_first_half)
        dataset_second_half = TextDataset(sequences_second_half)

        data_loader_first_half = DataLoader(dataset_first_half, batch_size=256, shuffle=True)
        data_loader_second_half = DataLoader(dataset_second_half, batch_size=256, shuffle=True)

        model = KnowledgeAugmentedLSTM(vocab_size)
        train_model(model, data_loader_first_half, num_epochs=5)

        train_model(model, data_loader_second_half, num_epochs=10)
    ...
    while True:
        ...
        print("AI:", generate_text(model, word_to_index, user_input, sequence_length=n, generate_length=generate_length, temperature=temperature))
