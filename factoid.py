...
KBfilename = "test.txt"
KB_MEM = 1200 # -1 for unlimited
num_epochs = 20

# Define the Agent class which will contain the neural network model
class Agent(nn.Module):
    def __init__(self, vocab_size, embedding_dim=12800, hidden_dim=12800, sequence_length=3):
...
# Prediction function
def generate_text(agent, env, input_text, sequence_length=4, max_length=50):
    ...
    return ' '.join([env.index_to_word.get(idx, str(idx)) for idx in generated_text])
...
def main():
    ...
    while True:
        ...
        print(f"Generated Text: {generate_text(agent, env, input_text, sequence_length=3, max_length=150).lower()}")
